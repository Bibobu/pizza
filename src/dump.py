...
import math  # any function could be used by set()
import numpy as np
...
class dump:
    ...
    def map(self, *pairs):
        if len(pairs) % 2 != 0:
            sys.exit("dump map() requires pairs of mappings")
    ...
    def sort(self, *args):
        if len(args) == 0:
            ...
            for snap in self.snaps:
                if snap.tselect:
                    self.sort_one(snap, atid)
        elif isinstance(args[0], str):
            print("Sorting selected snapshots by %s ..." % args[0])
            atid = self.names[args[0]]
            for snap in self.snaps:
                if snap.tselect:
                    self.sort_one(snap, atid)
        else:
            i = self.findtime(args[0])
            atid = self.names["id"]
            self.sort_one(self.snaps[i], atid)

    # --------------------------------------------------------------------
    # sort a single snapshot by ID column

    def sort_one(self, snap, atid):
        atoms = snap.atoms
        ids = atoms[:, atid]
        ...
        for snap in self.snaps:
            ...
            if header:
                ...
                if snap.boxstr:
                    ...
                    f.write("ITEM: BOX BOUNDS\n")
                if snap.triclinic:
                    f.write("{} {} {}".format(snap.xlo, snap.xhi, snap.xy))
                    f.write("{} {} {}".format(snap.ylo, snap.yhi, snap.xz))
                    f.write("{} {} {}".format(snap.zlo, snap.zhi, snap.yz))
                else:
                    f.write("{} {}".format(snap.xlo, snap.xhi))
                    f.write("{} {}".format(snap.ylo, snap.yhi))
                    f.write("{} {}".format(snap.zlo, snap.zhi))
                f.write("ITEM: ATOMS ")
                f.write("{}\n".format(namestr))
    ...
    def scatter(self, root):
        ...
        for snap in self.snaps:
            ...
            if snap.boxstr:
                ...
                f.write("ITEM: BOX BOUNDS\n")
            if snap.triclinic:
                f.write("{} {} {}\n".format(snap.xlo, snap.xhi, snap.xy))
                f.write("{} {} {}\n".format(snap.ylo, snap.yhi, snap.xz))
                f.write("{} {} {}\n".format(snap.zlo, snap.zhi, snap.yz))
            else:
                f.write("{} {}\n".format(snap.xlo, snap.xhi))
                f.write("{} {}\n".format(snap.ylo, snap.yhi))
                f.write("{} {}\n".format(snap.zlo, snap.zhi))
            f.write("ITEM: ATOMS ")
            f.write("{}\n".format(namestr))
    ...
    def minmax(self, colname):
        ...
        for snap in self.snaps:
            ...
            for i in range(snap.natoms):
                if not snap.aselect[i]:
                    continue
                if atoms[i][icol] < mini:
                    mini = atoms[i][icol]
                if atoms[i][icol] > maxi:
                    maxi = atoms[i][icol]
        return (mini, maxi)
    ...
    def setv(self, colname, vec):
        ...
        for snap in self.snaps:
            ...
            if snap.nselect != len(vec):
                sys.exit("vec length does not match # of selected atoms")
            atoms = snap.atoms
            m = 0
            for i in range(snap.natoms):
                if snap.aselect[i]:
    ...
    def clone(self, nstep, col):
        ...
        for i in range(self.snaps[istep].natoms):
            ids[self.snaps[istep].atoms[i][atid]] = i
        for snap in self.snaps:
            if not snap.tselect:
                continue
            atoms = snap.atoms
    ...
    # --------------------------------------------------------------------
    # values in old column are spread as ints from 1-N
    # and assigned to new column
    ...
    def spread(self, old, n, new):
        ...
        for snap in self.snaps:
            ...
            for i in range(snap.natoms):
                if not snap.aselect[i]:
                    continue
                ivalue = int((atoms[i][iold] - mini) * invdelta) + 1
                if ivalue > n:
                    ivalue = n
                if ivalue < 1:
                    ivalue = 1
    ...
    def atom(self, n, *args):
        if len(args) == 0:
            sys.exit("no columns specified")
        columns = []
        values = []
        for name in args:
        ...
        for snap in self.snaps:
            ...
            for i in range(snap.natoms):
                if atoms[i][atid] == n:
                    break
            if atoms[i][atid] != n:
                sys.exit("could not find atom ID in snapshot")
            for j in range(ncol):
    ...
    def newcolumn(self, string):
        ncol = len(self.snaps[0].atoms[0])
        self.map(ncol + 1, string)
        for snap in self.snaps:
            atoms = snap.atoms
    ...
    def viz(self, index, flag=0):
        ...
        for i in range(snap.natoms):
            if not snap.aselect[i]:
                continue
            atom = snap.atoms[i]
            atoms.append([atom[atid], atom[attype], atom[x], atom[y], atom[z]])
    ...
    def extra(self, arg):
        ...
        if type(arg) is types.InstanceType and ".data" in str(arg.__class__):
            ...
            try:
                ...
                sys.exit("could not extract bonds from data object")
        ...
        # cdata object, grab tris and lines statically
            ...
            try:
                ...
                sys.exit("could not extract tris/lines from cdata object")
        ...
        # tdump object, grab tris dynamically
            ...
            sys.exit("unrecognized argument to dump.extra()")
