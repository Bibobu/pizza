...
from tkinter import Tk, Toplevel, YES
from time import sleep
import OpenGL.GL as GL
import OpenGL.GLU as GLU
import OpenGL.GLUT as GLUT
from pyopengltk import OpenGLFrame
from pyopengltk import Opengl as MyOpengl
from PIL import Image
from vizinfo import vizinfo
from vizinfo import colors as vzcolors
...
class gl:
    ...
    def __init__(self, data, *args):
        ...
        if len(args) == 0:
            ...
            sys.exit("invalid args to gl tool")
        self.ztheta = 60
        self.azphi = 30
        self.scale = 1.0
        ...
        self.ready = 0
        # some versions of freeglut require this before any GLUT call
        GLUT.glutInit()
        self.create_window()
    ...
    def bg(self, color):
        ...
        self.bgcol = [
            vzcolors[color][0] / 255.0,
            vzcolors[color][1] / 255.0,
            vzcolors[color][2] / 255.0,
        ]
        self.w.tkRedraw()
    ...
    def create_window(self):
        ...
        self.w = MyOpengl(
            self.root, width=self.xpixels, height=self.ypixels
        )
        # self.w = MyOpengl(
        ...
        GL.glViewport(0, 0, self.xpixels, self.ypixels)
    ...
    def box(self, *args):
        ...
        if len(args) > 1:
            ...
            self.bxcol = [
                vzcolors[args[1]][0] / 255.0,
                vzcolors[args[1]][1] / 255.0,
                vzcolors[args[1]][2] / 255.0,
            ]
        if len(args) > 2:
            self.bxthick = args[2]
        self.cachelist = -self.cachelist
    ...
    def pan(self, *args):
        if len(args) == 0:
            ...
            self.panflag = 1
            self.ztheta_start = args[0]
            self.azphi_start = args[1]
            self.scale_start = args[2]
            self.ztheta_stop = args[3]
            self.azphi_stop = args[4]
            self.scale_stop = args[5]
    ...
    def redraw(self, o):
        ...
        if self.orthoflag:
            ...
            GLU.gluPerspective(30.0, 1.0, 0.01, 10000.0)
        ...
        if self.cache and self.cachelist > 0:
            GL.glCallList(self.cachelist)
        else:
            if self.cache:
                if self.cachelist < 0:
                    GL.glDeleteLists(-self.cachelist, 1)
                self.cachelist = GL.glGenLists(1)
                GL.glNewList(self.cachelist, GL.GL_COMPILE_AND_EXECUTE)
            ...
            if not self.clipflag:
                for atom in self.atomdraw:
                    GL.glTranslatef(atom[2], atom[3], atom[4])
                    GL.glCallList(self.calllist[int(atom[1])])
                    GL.glTranslatef(-atom[2], -atom[3], -atom[4])
                ...
                if self.bonddraw:
                    ...
                    for bond in self.bonddraw:
                        ...
                        itype = int(bond[1])
                        if itype > ncolor:
                            sys.exit("bond type too big")
                        red, green, blue = self.vizinfo.bcolor[itype]
                        rad = self.vizinfo.brad[itype]
                        GL.glPushMatrix()
                        GL.glTranslatef(bond[2], bond[3], bond[4])
                        GL.glRotatef(bond[11], bond[12], bond[13], 0.0)
                        GL.glMaterialfv(
                            GL.GL_FRONT_AND_BACK, GL.GL_EMISSION, [red, green, blue, 1.0]
                        )
                        GL.glMaterialf(GL.GL_FRONT_AND_BACK, GL.GL_SHININESS, self.shiny)
                        obj = GLU.gluNewQuadric()
                        GLU.gluCylinder(obj, rad, rad, bond[10], self.nsides, self.nsides)
                        GL.glPopMatrix()
                ...
                if self.tridraw:
                    ...
                    if fillflag != 1:
                        ...
                        for tri in self.tridraw:
                            itype = int(tri[1])
                            if itype > ncolor:
                                sys.exit("tri type too big")
                            red, green, blue = self.vizinfo.tcolor[itype]
                            GL.glMaterialfv(
                                GL.GL_FRONT_AND_BACK, GL.GL_EMISSION, [red, green, blue, 1.0]
                            )
                            GL.glMaterialf(GL.GL_FRONT_AND_BACK, GL.GL_SHININESS, self.shiny)
                            GL.glNormal3f(tri[11], tri[12], tri[13])
                            GL.glVertex3f(tri[2], tri[3], tri[4])
                            GL.glVertex3f(tri[5], tri[6], tri[7])
                            GL.glVertex3f(tri[8], tri[9], tri[10])
                        GL.glEnd()
                        if fillflag:
                    ...
                    if fillflag:
                        ...
                        GL.glLineWidth(self.bxthick)
                        GL.glColor3f(self.bxcol[0], self.bxcol[1], self.bxcol[2])
                        GL.glBegin(GL.GL_TRIANGLES)
                        for tri in self.tridraw:
                            GL.glVertex3f(tri[2], tri[3], tri[4])
                            GL.glVertex3f(tri[5], tri[6], tri[7])
    ...
    def make_atom_calllist(self):
        ...
        for itype in range(1, self.vizinfo.nacolor + 1):
            if self.calllist[itype]:
                GL.glDeleteLists(self.calllist[itype], 1)
            ...
            GL.glPointSize(10.0*rad)
            GL.glBegin(GL.GL_POINTS)
            GL.glVertex3f(0.0, 0.0, 0.0)
            GL.glEnd()

            # GL.glMaterialfv(GL.GL_FRONT, GL.GL_EMISSION, [red, green, blue, 1.0])
            # GL.glMaterialf(GL.GL_FRONT, GL.GL_SHININESS, self.shiny)
            # # GLUT.glutSolidSphere(rad, self.nslices, self.nstacks)
            # GLU.gluSphere(sphere, rad, self.nslices, self.nstacks)
            GL.glEndList()
    ...
    def draw_box(self, flag):
        ...
        GL.glVertex3f(xlo, ylo, zlo)
        GL.glVertex3f(xhi, ylo, zlo)
        GL.glVertex3f(xhi, yhi, zlo)
        GL.glVertex3f(xlo, yhi, zlo)
        GL.glEnd()

        GL.glBegin(GL.GL_LINE_LOOP)
        GL.glVertex3f(xlo, ylo, zhi)
        GL.glVertex3f(xhi, ylo, zhi)
        GL.glVertex3f(xhi, yhi, zhi)
        GL.glVertex3f(xlo, yhi, zhi)
        GL.glEnd()
    ...
    def draw_axes(self):
        ...
        GL.glBegin(GL.GL_LINES)
        GL.glColor3f(1, 0, 0)
        GL.glVertex3f(xlo - delta, ylo - delta, zlo - delta)
        GL.glVertex3f(xhi - delta, ylo - delta, zlo - delta)
        GL.glColor3f(0, 1, 0)
        GL.glVertex3f(xlo - delta, ylo - delta, zlo - delta)
        GL.glVertex3f(xlo - delta, yhi - delta, zlo - delta)
        GL.glColor3f(0, 0, 1)
        GL.glVertex3f(xlo - delta, ylo - delta, zlo - delta)
        GL.glVertex3f(xlo - delta, ylo - delta, zhi - delta)
        GL.glEnd()
    ...
    def save(self, file=None):
        ...
        pstring = GL.glReadPixels(
            0, 0, self.xpixels, self.ypixels, GL.GL_RGBA, GL.GL_UNSIGNED_BYTE
        )
