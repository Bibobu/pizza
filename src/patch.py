...
class patch:
    ...
    def write3d(self, file):
        ...
        if self.lattice[0] or self.lattice[1] or self.lattice[2]:
            ...
            if self.lattice[0] * self.lattice[1] * self.lattice[2] != len(
                ...
                sys.exit("lattice inconsistent with # of molecules")
        else:
            latflag = 0
        ...
        while self.molecules:
            ...
            # unpack bonds in molecule before atoms so
            # idatom = all previous atoms
        ...
        if tris:
            print(" ".join(["TRIS", len(tris), d.headers["triangles"]]))
    ...
    def write2d(self, file):
        ...
        while self.molecules:
            ...
            # subs = molecule[4] field = sub-particle displacements
            # from body atom
        ...
        if bodies:
            ...
            for body in bodies:
                ...
                i = 0
                while i < len(ivalues):
                    values_list = ivalues[i:min(i+10, len(ivalues)+1)]
                    one = ""
                ...
                i = 0
                while i < len(dvalues):
                    values_list = dvalues[i:min(i+10, len(dvalues)+1)]
                    one = ""
                    for value in values_list:
                        one += "%g " % float(value)
                    one += "\n"
    ...
    # m12,m12type = length of tethers on each side of ball
    # (m12 = 0 = no tether)
    ...
    def star2d(self, *params):
        ...
        statype = params[2]
        if n % 2 == 0:
            sys.exit("N in patch::star2d is not odd")
        middle = n / 2

        atoms = []
        x, y, z = 0.0, 0.0, 0.0
        atoms.append([statype, x, y, z])
        for i in range(n):
            i -= middle
            if i == 0:
                continue
    ...
    def pgon2d(self, *params):
        ...
        for i in range(n):
            theta = 2.0 * pi * i / n
            parts.append([rad * cos(theta), rad * sin(theta), 0])
        ...
        atoms = []
        for one in parts:
            atoms.append([pgtype, one[0], one[1], 0.0])
    ...
    def sphere3d(self, *params):
        ...
        for i in range(n):
            for j in range(n):
                ...
                atoms.append([stype, scale * c[0], scale * c[1], scale * c[2]])
                atoms.append([stype, scale * c[0], scale * c[1], -scale * c[2]])
        for i in range(n):
    ...
    def linebox(self, *params):
        ...
        atoms.append([litype, 0, -b / 2, 0])
        atoms.append([litype, a / 2, 0, 0])
        atoms.append([litype, 0, b / 2, 0])
        atoms.append([litype, -a / 2, 0, 0])
    ...
    def linetri(self, *params):
        ...
        atoms.append([litype, 0, -ht / 2, 0])
        atoms.append([litype, base / 4, 0, 0])
        atoms.append([litype, -base / 4, 0, 0])
...
def atom_on_sphere(diam, ty, x, y, z):
    if x == 0.0 and y == 0.0 and z == 0.0:
        scale = 0.0
    else:
        scale = 0.5 * diam / sqrt(x * x + y * y + z * z)
    return [ty, scale * x, scale * y, scale * z]
...
def make_sphere(template, diam, type0, patches):
    ...
    for atom in template:
        ...
        for pair in patches:
            ...
            if n in patch:
                ty = pair[0]
        atoms.append(atom_on_sphere(diam, ty, atom[0], atom[1], atom[2]))
    return atoms
