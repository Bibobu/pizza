...
# from tkinter import Tk, Toplevel, Frame, LEFT, HORIZONTAL, Scale
...
class vcr:
    ...
    def __init__(self, *views):
        ...
        if len(views) == 0:
            sys.exit("must have at least one GL viewer")
        self.viewlist = []
        for view in views:
...
#         from __main__ import tkroot
        ...
        self.tkroot = tk.Tk()
        root = tk.Toplevel(self.tkroot)
        root.title("Pizza.py vcr tool")

        frame1 = tk.Frame(root)
        tk.Button(frame1, text="<<", command=self.first).pack(side=tk.LEFT)
        tk.Button(frame1, text="<", command=self.previous).pack(side=tk.LEFT)
        tk.Button(frame1, text="Back", command=self.back).pack(side=tk.LEFT)
        tk.Button(frame1, text="Stop", command=self.stop).pack(side=tk.LEFT)
        tk.Button(frame1, text="Play", command=self.play).pack(side=tk.LEFT)
        tk.Button(frame1, text=">", command=self.next).pack(side=tk.LEFT)
        tk.Button(frame1, text=">>", command=self.last).pack(side=tk.LEFT)
        frame1.pack()
        ...
        self.slider_quality = tk.Scale(
            frame2, from_=3, to=12, orient=tk.HORIZONTAL, label="       Quality"
        )
        ...
        self.slider_delay.pack(side=tk.LEFT)
        frame2.pack()

        frame3 = tk.Frame(root)
        self.slider_xlo = tk.Scale(
            frame3,
            from_=0.0,
            to=1.0,
            resolution=0.01,
        ...
        self.slider_xlo.pack(side=tk.LEFT)
        self.slider_ylo.pack(side=tk.LEFT)
        self.slider_zlo.pack(side=tk.LEFT)
        frame3.pack()

        frame4 = tk.Frame(root)
        self.slider_xhi = tk.Scale(
            frame4,
            from_=0.0,
            to=1.0,
            resolution=0.01,
        ...
        self.slider_xhi.pack(side=tk.LEFT)
        self.slider_yhi.pack(side=tk.LEFT)
        self.slider_zhi.pack(side=tk.LEFT)
        frame4.pack()

        frame5 = tk.Frame(root)
        tk.Button(frame5, text="X", command=self.xaxis).pack(side=tk.LEFT)
        tk.Button(frame5, text="Y", command=self.yaxis).pack(side=tk.LEFT)
        tk.Button(frame5, text="Z", command=self.zaxis).pack(side=tk.LEFT)
        tk.Button(frame5, text="Box", command=self.box).pack(side=tk.LEFT)
        tk.Button(frame5, text="Axes", command=self.axis).pack(side=tk.LEFT)
        tk.Button(frame5, text="Recenter", command=self.recenter).pack(side=tk.LEFT)
        self.button_ortho = tk.Button(frame5, text="Persp", command=self.ortho)
        self.button_ortho.pack(side=tk.LEFT)
        tk.Button(frame5, text="Reload", command=self.reload).pack(side=tk.LEFT)
        frame5.pack()

        frame6 = tk.Frame(root)
        tk.Button(frame6, text="Save As:", command=self.save).pack(side=tk.LEFT)
        self.entry_file = tk.Entry(frame6, width=16)
        self.entry_file.insert(0, "image")
        self.entry_file.pack(side=tk.LEFT)
        self.button_save = tk.Checkbutton(frame6, text="SaveAll", command=self.saveall)
        self.button_save.pack(side=tk.LEFT)
        frame6.pack()

        frame7 = tk.Frame(root)
        tk.Label(frame7, text="Mouse LMR = Shift/Rotate/Zoom").pack(padx=5, side=tk.LEFT)
        tk.Label(frame7, text="Axes XYZ = Red/Green/Blue").pack(padx=5, side=tk.LEFT)
        frame7.pack()

        frame8 = tk.Frame(root)
        self.label_frame = tk.Label(frame8, text="Frame 0")
        self.label_frame.pack(padx=5, side=tk.LEFT)
        self.label_time = tk.Label(frame8, text="Time 0")
        self.label_time.pack(padx=5, side=tk.LEFT)
        self.label_atoms = tk.Label(frame8, text="Atoms 0")
        self.label_atoms.pack(padx=5, side=tk.LEFT)
        frame8.pack()
