...
class data:
    ...
    def __init__(self, *args):
        ...
        while 1:
            ...
            if not found:
                sys.exit("invalid section %s in data file" % line)
            f.readline()
            line = f.readline()
            if not line:
                break
    ...
    def map(self, *pairs):
        if len(pairs) % 2 != 0:
            sys.exit("data map() requires pairs of mappings")
    ...
    def get(self, *args):
        if len(args) == 1:
            ...
            sys.exit("invalid arguments for data.get()")
    ...
    def delete(self, keyword):
        ...
        if keyword in self.headers:
            ...
            sys.exit("keyword not found in data object")
        ...
        for keyword in hkeywords:
            if keyword in self.headers:
                if keyword == "xlo xhi" or keyword == "ylo yhi" or keyword == "zlo zhi":
                    ...
                    f.write("{} {} {}\n".format(pair[0], pair[1], keyword))
                elif keyword == "xy xz yz":
                    triple = self.headers[keyword]
                    f.write("{} {} {} {}\n".format(
                        triple[0], triple[1], triple[2], keyword
                        )
                    )
                else:
    ...
    def viz(self, isnap):
        if isnap:
            sys.exit("cannot call data.viz() with isnap != 0")

        ids = self.names["id"]
        types = self.names["type"]
        x = self.names["x"]
        y = self.names["y"]
        z = self.names["z"]
            ...
            for line in bondlines:
                ...
                bonds.append(
                    [
                        ...
                        float(atom2words[y]),
                        float(atom2words[z]),
                        float(atom1words[types]),
                        float(atom2words[types]),
                    ]
                )
