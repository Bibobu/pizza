...
class cdata:
        ...
        for file in flist:
            ...
            while 1:
                ...
                if not line:
                    ...
                    sys.exit("unrecognized ChemCell data file")

                # create a surface object from set of triangles or facets

                if flag == "triangles" or flag == "facets":
                    tmp, atid, nvert, ntri = line.split()
                    nvert = int(nvert)
                    ntri = int(ntri)

                    if atid in self.ids:
                        sys.exit("ID %s is already in use" % atid)

                    f.readline()
                    vertices = []
                    for i in range(nvert):
                        values = f.readline().split()
                        vertices.append([float(value) for value in values[1:]])
                    f.readline()
                    triangles = []
                    for i in range(ntri):
                        values = f.readline().split()
                        triangles.append([int(value) for value in values[1:]])
                    ...
                    if flag == "triangles":
                        ...
                        for i in range(ntri):
                            values = f.readline().split()
                            connections.append([int(value) for value in values[1:]])
                    else:
                        connections = connect(nvert, ntri, triangles)

                    obj = Surface()
                    obj.select = 1
                    self.ids[atid] = len(self.objs)
                ...
                if flag == "particles":
                    ...
                    for i in range(npart):
                        values = f.readline().split()
                        xyz.append([float(value) for value in values[1:]])
    ...
    def box(self, bid, *args):
        if bid in self.ids:
            sys.exit("ID %s is already in use" % bid)
        obj = Box(*args)
        obj.select = 1
        self.ids[bid] = len(self.objs)
        self.objs.append(obj)
        obj.id = bid
        obj.style = REGION
        obj.substyle = BOX
    ...
    def sphere(self, sid, *args):
        if sid in self.ids:
            sys.exit("ID %s is already in use" % sid)
        obj = Sphere(*args)
        obj.select = 1
        self.ids[sid] = len(self.objs)
        self.objs.append(obj)
        obj.id = sid
        obj.style = REGION
        obj.substyle = SPHERE
    ...
    def shell(self, shid, *args):
        if shid in self.ids:
            sys.exit("ID %s is already in use" % shid)
        obj = Shell(*args)
        obj.select = 1
        self.ids[shid] = len(self.objs)
        self.objs.append(obj)
        obj.id = shid
        obj.style = REGION
        obj.substyle = SHELL
    ...
    def cyl(self, cid, *args):
        if cid in self.ids:
            sys.exit("ID %s is already in use" % cid)
        obj = Cylinder(*args)
        obj.select = 1
        self.ids[cid] = len(self.objs)
        self.objs.append(obj)
        obj.id = cid
        obj.style = REGION
    ...
    def q(self, qid, *args):
        obj = self.objs[self.ids[qid]]
        if obj.style != REGION:
            sys.exit("Can only use q() on a region object")
        n = 1
        for arg in args:
            cmd = "obj.q%d = arg" % n
            exec(cmd)
    ...
    def line(self, lid, *args):
        if lid in self.ids:
            sys.exit("ID %s is already in use" % lid)
        obj = Line()
        obj.select = 1
        self.ids[lid] = len(self.objs)
        self.objs.append(obj)
        obj.id = lid
        obj.style = LINE
        obj.nline = 0
    ...
    def surftri(self, sid, id_surf, *args):
        if sid in self.ids:
            sys.exit("ID %s is already in use" % sid)
    ...
    def surfselect(self, sid, id_surf, teststr):
        if sid in self.ids:
            sys.exit("ID %s is already in use" % sid)
    ...
    def bins(self, bid, nx, ny):
        obj = self.objs[self.ids[bid]]
        if obj.style != SURFACE:
            sys.exit("Can only set bins for surface")
        obj.nbinx = nx
        obj.nbiny = ny
    ...
    def part(self, pid, npart, in_id, out_id=None):
        if pid in self.ids:
            sys.exit("ID %s is already in use" % pid)

        obj = Group()
        obj.select = 1
        self.ids[pid] = len(self.objs)
        self.objs.append(obj)
        obj.id = pid
        obj.style = GROUP
        obj.on_id = ""
    ...
    def part2d(self, pid, npart, on_id):
        if pid in self.ids:
            sys.exit("ID %s is already in use" % pid)
    ...
    def partarray(self, pid, nx, ny, nz, x, y, z, dx, dy, dz):
        if pid in self.ids:
            sys.exit("ID %s is already in use" % pid)
    ...
    def partring(self, pid, n, x, y, z, r, axis):
        if pid in self.ids:
            sys.exit("ID %s is already in use" % pid)
    ...
    def partsurf(self, pid, on_id):
        obj = self.objs[self.ids[pid]]
        if obj.style != GROUP:
            sys.exit("Must use particle group with partsurf()")
        if obj.on_id == "":
            sys.exit("Must use partsurf() with 2d particles")
        obj.on_id = on_id
    ...
    def random(self, pid):
        ...
        if obj.style != SURFACE and obj.style != REGION:
            sys.exit("Must use surf or region with random()")
    ...
    def project(self, pid, id2, dx, dy, dz, EPS, flag=None):
        obj = self.objs[self.ids[pid]]
        if obj.style != GROUP:
            sys.exit("Must use particle group as 1st obj of project()")
        obj_on = self.objs[self.ids[id2]]
        if obj_on.style != SURFACE and obj_on.style != REGION:
            sys.exit("Must use surf or region as 2nd obj of project()")
        ...
        for i in range(obj.npart):
            ...
            if bracket == start:
                sys.exit("Could not find bracket point for particle %d" % i)
    ...
    def center(self, cid, x, y, z):
        obj = self.objs[self.ids[cid]]
        if obj.style != SURFACE and obj.style != GROUP:
            sys.exit("Can only use center() on a surface or group object")
        obj.center(x, y, z)
    ...
    def trans(self, tid, dx, dy, dz):
        obj = self.objs[self.ids[tid]]
        if obj.style != SURFACE and obj.style != GROUP:
            sys.exit("Can only use trans() on a surface or group object")
        obj.xc += dx
        obj.yc += dy
        obj.zc += dz
    ...
    def rotate(self, rid, axis1, i1, j1, k1, axis2, i2, j2, k2):
        obj = self.objs[self.ids[rid]]
        if obj.style != SURFACE and obj.style != GROUP:
            sys.exit("Can only use rotate() on a surface or group object")
        ...
        if axis1 == "x":
            ...
            sys.exit("Illegal rotate arguments")
        if axis2 == "x":
            xnew = [i2, j2, k2]
        elif axis2 == "y":
            ynew = [i2, j2, k2]
    ...
    def scale(self, sid, sx, sy, sz):
        obj = self.objs[self.ids[sid]]
        if obj.style != SURFACE and obj.style != GROUP:
            sys.exit("Can only use scale() on a surface or group object")
        if obj.style == SURFACE:
            for i in range(obj.nvert):
    ...
    def union(self, uid, *args):
        if uid in self.ids:
            sys.exit("ID %s is already in use" % uid)
        obj = Union(self.ids, self.objs, *args)
        obj.select = 1
        self.ids[uid] = len(self.objs)
        self.objs.append(obj)
        obj.id = uid
        obj.style = UNION
    ...
    def join(self, jid, *args):
        style = self.objs[self.ids[args[0]]].style
        if style == GROUP:
            obj = Group()
        elif style == SURFACE:
            obj = Surface()
            ...
            sys.exit("Cannot perform join on these object styles")
        obj.select = 1
        self.ids[jid] = len(self.objs)
        self.objs.append(obj)
        obj.id = jid
        obj.style = style

        if style == GROUP:
            obj.on_id = self.objs[self.ids[args[0]]].on_id
            obj.npart = 0
            obj.xyz = []
        elif style == SURFACE:
        ...
        for jid in args:
            o = self.objs[self.ids[jid]]
            if o.style != style:
                sys.exit("All joined objects must be of same style")

            # force deep copy of particle coords

            if style == GROUP:
                if o.on_id != obj.on_id:
                    sys.exit("Particle group surfaces do not match")
    ...
    def rename(self, idold, idnew):
        ...
        if idnew in self.ids:
            sys.exit("ID %s is already in use" % idnew)
        self.ids[idnew] = i
    ...
    def write(self, file, *args):
        if not len(args):
            vlist = range(len(self.objs))
        else:
            vlist = []
            for i in args:
                vlist.append(self.ids[i])
    ...
    def filewrite(self, fp, vlist):
        for index in vlist:
            ...
            if obj.style == GROUP:
                if not obj.on_id:
                    fp.write("particles %s %d\n" % (obj.id, obj.npart))
                else:
                    fp.write("particles %s %d %s\n" % (obj.id, obj.npart, obj.on_id))
                for i in range(obj.npart):
                    fp.write("{:>} {:>} {:>} {:>}\n".format(
                        i + 1, obj.xyz[i][0], obj.xyz[i][1], obj.xyz[i][2]
                        )
                    )
            if obj.style == SURFACE:
                fp.write("triangles %s %d %d\n" % (obj.id, obj.nvert, obj.ntri))
                for i in range(obj.nvert):
                    fp.write("{:>} {:>} {:>} {:>}\n".format(
                        i + 1, obj.vertices[i][0], obj.vertices[i][1], obj.vertices[i][2]
                        )
                    )
                for i in range(obj.ntri):
                    fp.write("{:>} {:>} {:>} {:>}\n".format(
                        i + 1, obj.triangles[i][0], obj.triangles[i][1], obj.triangles[i][2]
                        )
                    )
                for i in range(obj.ntri):
                    fp.write("{:>} {:>} {:>} {:>} {:>} {:>} {:>}\n".format(
                        i + 1, obj.connections[i][0],
                        obj.connections[i][1], obj.connections[i][2],
                        obj.connections[i][3], obj.connections[i][4],
                        obj.connections[i][5]
                        )
                    )
            if obj.style == REGION:
    ...
    def viz(self, isnap):
        if isnap:
            sys.exit("cannot call cdata.viz() with isnap != 0")
        ...
        for obj in self.objs:
            ...
            for xyz in obj.xyz:
                atid += 1
                atoms.append([atid, itype, xyz[0], xyz[1], xyz[2]])
        ...
        for obj in self.objs:
            ...
            for tri in obj.triangles:
                ...
                verts = v1 + v2 + v3
                n = normal(verts[0:3], verts[3:6], verts[6:9])
                tid += 1
                tris.append([tid, itype] + verts + n)
...
class Surface:
    ...
    def bbox(self):
        xbound = [float(vert[0]) for vert in self.vertices]
        xlo = min(xbound)
        xhi = max(xbound)
        ybound = [float(vert[1]) for vert in self.vertices]
        ylo = min(ybound)
        yhi = max(ybound)
        zbound = [float(vert[2]) for vert in self.vertices]
        zlo = min(zbound)
        zhi = max(zbound)
    ...
    def area(self):
        ...
        for tri in self.triangles:
            ...
            s += 0.5 * sqrt(a[0] * a[0] + a[1] * a[1] + a[2] * a[2])
            self.areas.append(s)
        return s
    ...
    def loc2d(self, area, random):
        ...
        verts = v1 + v2 + v3
        return [x, y, z], normal(verts[0:3], verts[3:6], verts[6:9])
...
class Group:
    def bbox(self):
        xbound = [xyz[0] for xyz in self.xyz]
        xlo = min(xbound)
        xhi = max(xbound)
        ybound = [xyz[1] for xyz in self.xyz]
        ylo = min(ybound)
        yhi = max(ybound)
        zbound = [xyz[2] for xyz in self.xyz]
        zlo = min(zbound)
        zhi = max(zbound)
...
class Cylinder:
    ...
    def area(self):
        ...
        area += pi * self.rsq
        self.areas.append(area)
        area += 2 * pi * self.r * (self.hi - self.lo)
        self.areas.append(area)
        return area
...
class Capped:
    ...
    def triangulate(self):
        if self.q1 % 2:
            sys.exit("Capped cylinder q1 must be even")
...
class Line:
    def bbox(self):
        verts = [pair[0] for pair in self.pairs]
        verts += [pair[3] for pair in self.pairs]
        xlo = min(verts)
        xhi = max(verts)
        verts = [pair[1] for pair in self.pairs]
        verts += [pair[4] for pair in self.pairs]
        ylo = min(verts)
        yhi = max(verts)
        verts = [pair[2] for pair in self.pairs]
        verts += [pair[5] for pair in self.pairs]
        zlo = min(verts)
        zhi = max(verts)
        return (xlo, ylo, zlo, xhi, yhi, zhi)
...
class Union:
    def __init__(self, ids, objs, *args):
        self.objs = []
        for oid in args:
            obj = objs[ids[oid]]
            if obj.style != SURFACE and obj.style != REGION and obj.style != UNION:
                sys.exit("Union child object is of invalid style")
            self.objs.append(obj)
