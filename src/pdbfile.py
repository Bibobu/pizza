...
class pdbfile:
        ...
        if filestr:
            ...
            for i in range(len(flist)):
                if flist[i][-4:] != ".pdb":
                    flist[i] += ".pdb"
            if len(flist) == 0:
                sys.exit("no PDB file specified")
            self.files = flist
        else:
            self.files = []

        if len(self.files) > 1 and self.data:
            sys.exit("cannot use multiple PDB files with data object")
        if len(self.files) == 0 and not self.data:
            sys.exit("no input PDB file(s)")
    ...
    def convert(self, f, which):
        ...
        if len(self.files):
            for atom in atoms:
                atid = atom[0]
                if atid in self.atomlines:
                    (begin, end) = self.atomlines[atid]
