...
"""
Read, create, manipulate SPARTA/LAMMPS surf files
...
class sdata:
    ...
    def __init__(self, *list):
        ...
        if len(list) == 1:
            sys.exit("surf ID and surf file required")
        if len(list) > 1:
            self.read(list[0], *list[1:])

    # --------------------------------------------------------------------

    def read(self, fid, *args):
        ...
        if len(flist) == 0 and len(args) == 1:
            sys.exit("no surf file specified")
        ...
        for file in flist:
            ...
            while 1:
                ...
                if "points" in line:
                    ...
                    if nlines:
                        if self.dim == 3:
                            sys.exit("cannot have both 2d/3d surfs")
                        self.dim = 2
                    if ntriangles:
                        if self.dim == 2:
                            sys.exit("cannot have both 2d/3d surfs")
                    ...
                    for i in range(npoints):
                        ...
                        pt = [float(value) for value in point[1:]]
                        if len(pt) == 2:
        ...
        if self.dim == 2:
            print("read surf %s with %d points, %d lines" % (
                fid,
                len(points),
                len(lines),
                )
        ...
        if self.dim == 3:
            print("read surf %s with %d points, %d triangles" % (
                fid,
                len(points),
                len(triangles),
                )
        ...
        self.ids[fid] = len(self.surfs)
        self.surfs.append(surf)

    # --------------------------------------------------------------------
    # create a circle

    def circle(self, cid, x, y, r, n):
        if self.ids.has_key(cid):
            sys.exit("ID %s is already in use" % id)
        if self.dim == 3:
            sys.exit("cannot have both 2d/3d surfs")
        self.dim = 2
        ...
        surf.lines = lines
        surf.center = [x, y, 0.0]
        self.ids[cid] = len(self.surfs)
        self.surfs.append(surf)

    # --------------------------------------------------------------------
    # create a rectangle

    def rect(self, rid, x0, y0, x1, y1, nx, ny):
        if rid in self.ids:
            sys.exit("ID %s is already in use" % rid)
        if self.dim == 3:
            sys.exit("cannot have both 2d/3d surfs")
        self.dim = 2
        ...
        self.ids[rid] = len(self.surfs)
        self.surfs.append(surf)

    # --------------------------------------------------------------------
    # create a triangle

    def tri(self, tid, x0, y0, x1, y1, x2, y2, n1, n2, n3):
        if tid in self.ids:
            sys.exit("ID %s is already in use" % tid)
        if self.dim == 3:
            sys.exit("cannot have both 2d/3d surfs")
        self.dim = 2
        ...
        self.ids[tid] = len(self.surfs)
        self.surfs.append(surf)

    # --------------------------------------------------------------------
    # create a sphere

    def sphere(self, sid, x, y, z, r, n):
        if sid in self.ids:
        ...
        surf.center = [x, y, z]
        self.ids[sid] = len(self.surfs)
        self.surfs.append(surf)

    # --------------------------------------------------------------------
    # create a 3d box

    def box(self, bid, x0, y0, z0, x1, y1, z1, nx, ny, nz):
        if bid in self.ids:
            sys.exit("ID %s is already in use" % bid)
        if self.dim == 2:
            sys.exit("cannot have both 2d/3d surfs")
        if nx <= 0 or ny <= 0 or nz <= 0:
            sys.exit("invalid box nx,ny,nz values")
        self.dim = 3
        ...
        surf.points = points
        surf.triangles = triangles
        surf.triflags = [[] for i, _ in enumerate(triangles)]
        surf.center = [0.5 * (x0 + x1), 0.5 * (y0 + y1), 0.5 * (z0 + z1)]
        self.ids[bid] = len(self.surfs)
        self.surfs.append(surf)

    # --------------------------------------------------------------------
    # create a spiky circle

    def spikycircle(self, sid, x, y, rmin, rmax, n):
        if sid in self.ids:
            sys.exit("ID %s is already in use" % sid)
        if self.dim == 3:
            sys.exit("cannot have both 2d/3d surfs")
    ...
    def spikysphere(self, sid, x, y, z, rmin, rmax, n):
        if sid in self.ids:
            sys.exit("ID %s is already in use" % sid)
        if self.dim == 2:
            sys.exit("cannot have both 2d/3d surfs")
    ...
    def surf2d(self, sid, plist, llist):
        if sid in self.ids:
            sys.exit("ID %s is already in use" % sid)
        if self.dim == 3:
            sys.exit("cannot have both 2d/3d surfs")
    ...
    def surf3d(self, sid, plist, tlist):
        if sid in self.ids:
            sys.exit("ID %s is already in use" % sid)
        if self.dim == 2:
            sys.exit("cannot have both 2d/3d surfs")
    ...
    def center(self, cid, x, y, z):
        if cid not in self.ids:
            sys.exit("ID %s is not defined" % cid)
        if self.dim == 2 and z != 0.0:
            sys.exit("z center of 2d surf must be 0.0")
        surf = self.surfs[self.ids[cid]]
        surf.center = [x, y, z]
    ...
    def trans(self, sid, dx, dy, dz):
        if sid not in self.ids:
            sys.exit("ID %s is not defined" % sid)
        if self.dim == 2 and dz != 0.0:
            sys.exit("dz translation of 2d surf must be 0.0")

        surf = self.surfs[self.ids[sid]]
        surf.xc += dx
        surf.yc += dy
        surf.zc += dz
    ...
    def rotate(self, sid, theta, rx, ry, rz):
        if sid not in self.ids:
            sys.exit("ID %s is not defined" % sid)
        if self.dim == 2 and (rx != 0.0 or ry != 0.0):
            sys.exit("rx,ry rotation of 2d surf must be 0.0")
    ...
    def scale(self, sid, sx, sy, sz):
        ...
        if self.dim == 2 and sz != 1.0:
            sys.exit("sz scale of 2d surf must be 1.0")
    ...
    def join(self, sid, *args):
        if sid in self.ids:
            sys.exit("ID %s is already in use" % sid)
        if len(args) == 0:
            sys.exit("list of surfs to join is empty")
    ...
    def write(self, file, pflag=1, nfile=0, ids=[]):
        if pflag != 0 and pflag != 1:
            sys.exit("Pflag value is invalid")
        if nfile < 0:
            sys.exit("Nfile value is invalid")
        if pflag == 1 and nfile:
            sys.exit("Cannot use pflag=1 with nfile >= 1")
        if pflag == 0 and self.mode == "LAMMPS":
            sys.exit("Cannot use pflag=0 for LAMMPS mode")
        ...
        if pflag:
            if self.dim == 2:
                print("wrote surf file with %d points, %d lines" % (
                    len(points),
                    len(lines),
                    )
                )
            if self.dim == 3:
                print("wrote surf file with %d points, %d triangles" % (
                    len(points),
                    len(triangles),
                    )
                )
    ...
    def write_single(self, file, pflag, points, lines, lineflags, triangles, triflags):
        ...
        fp.write("surf file from Pizza.py\n")
        fp.write("{} points\n".format(len(points)))
        if self.dim == 2:
            fp.write("{} lines\n".format(len(lines)))
        if self.dim == 3:
    ...
    def write_parallel(
        ...
        basefile = file + ".base"
        fp = open(basefile, "w")
        fp.write("multiproc surf file from Pizza.py\n")
        fp.write("\n")
        fp.write("{} files\n".format(nfile))
        if self.dim == 2:
            fp.write("{} lines\n".format(len(lines)))
            ...
            if self.dim == 2:
                ...
                if lineflags[0]:
                    for i, line in enumerate(lines[istart:istop]):
                        fp.write("{} {} {} {} {} {}\n".format(
                            i+istart+1, " ".join( lineflags[i + istart]),
                            points[line[0]][0], points[line[0]][1],
                            points[line[1]][0], points[line[1]][1]
                            )
                        )
            ...
            if self.dim == 3:
                ...
                if triflags[0]:
                    for i, tri in enumerate(triangles[istart:istop]):
                        fp.write("{} {} {} {} {} {} {} {} {} {} {}\n".format(
                            i+istart+1, " ".join( triflags[i + istart]),
                            points[tri[0]][0], points[tri[0]][1], points[tri[0]][2],
                            points[tri[1]][0], points[tri[1]][1], points[tri[1]][2],
                            points[tri[2]][0], points[tri[2]][1], points[tri[2]][2]
                            )
    ...
    def grid(self, *args):
        if self.dim == 0:
            sys.exit("dimension must be defined for grid")
        if self.dim == 2 and len(args) != 6:
            sys.exit("bad arguments for sdata.grid()")
    ...
    def gridfile(self, *args):
        if self.mode != "SPARTA":
            sys.exit("gridfile() can only be used with SPARTA mode")
        if self.dim == 0:
            sys.exit("dimension must be defined for gridfile")
        if self.dim == 2 and len(args) != 5:
            sys.exit("bad arguments for sdata.gridfile()")
    ...
    def viz(self, isnap):
        if isnap:
            sys.exit("cannot call sdata.viz() with isnap != 0")
        ...
        if self.dim == 3:
            for surf in self.surfs:
                ...
                for tri in triangles:
                    ...
                    tris.append([typeid, itype] + pt1 + pt2 + pt3 + n)
        ...
        if self.dim == 2:
            for surf in self.surfs:
                ...
                for segment in segments:
                    typeid += 1
                    lines.append([typeid, itype] + points[segment[0]] + points[segment[1]])
        ...
        if self.gridflag and self.dim == 2:
            for idparent in self.idparents:
                ...
                for i in range(nx + 1):
                    ...
                    lines.append(
                        [typeid + i, itype + nlevel + 1] + [x, ylo, 0.0, x, yhi, 0.0]
                    )
                typeid += nx + 1
                for i in range(ny + 1):
                    y = ylo + float(i) * (yhi - ylo) / ny
                    lines.append(
                        [typeid + i, itype + nlevel + 1] + [xlo, y, 0.0, xhi, y, 0.0]
                    )
                typeid += ny + 1
            ...
            for idparent in self.idparents:
                ...
                for i in range(nx + 1):
                    ...
                    for j in range(nz + 1):
                        z = zlo + float(j) * (zhi - zlo) / nz
                        lines.append(
                            [typeid + i, itype + nlevel + 1] + [x, ylo, z, x, yhi, z]
                        )
                typeid += (nx + 1) * (nz + 1)
                for i in range(nx + 1):
                    x = xlo + float(i) * (xhi - xlo) / nx
                    for j in range(ny + 1):
                        ...
                        lines.append(
                            [typeid + i, itype + nlevel + 1] + [x, y, zlo, x, y, zhi]
                        )
                typeid += (nx + 1) * (ny + 1)
...
def connect_triangles(tris):
    ...
    for i, tri in enumerate(tris):
        edge = (tri[0], tri[1])
        if edge in h:
            sys.exit("surf connect has duplicate edge")
        h[edge] = (i, 1)
        edge = (tri[1], tri[2])
        if edge in h:
            sys.exit("surf connect has duplicate edge")
