...
import subprocess
import re
from vizinfo import vizinfo, colors as vzcolors
...
class svg:
    ...
    def label(self, x, y, font, point, color, text):
        ...
        label = [x, y, fontlist[font], point, scaledcolor, text]
    ...
    def write(self, f, flag, *args):
        ...
        if flag == 0:
            if obj[0] == 0:  # atom with its color and radius
                ...
                f.write('<circle cx="%s" cy="%s" r="%s" fill="rgb(%s,%s,%s)" stroke-width="%s" />\n' % (
                    obj[2],
                    obj[3],
                    rad * self.factor,
                    color[0] * 255,
                ...
                f.write('<polygon points= "%s,%s %s,%s %s,%s" fill="rgb(%s,%s,%s)" stroke="black" stroke-width="0.01" />\n' % (
                    obj[2],
                    obj[3],
                    obj[5],
                    obj[6],
                ...
                f.write('<line x1="%s" y1="%s" x2="%s" y2="%s" stroke="rgb(%s,%s,%s)" stroke-width="%s" />\n' % (
                    obj[2],
                    obj[3],
                    obj[5],
                    obj[6],
            ...
            color = obj[4]
            f.write('<text x="%s" y="%s" font-size="%s" font-family="%s" stroke="rgb(%s,%s,%s)" fill="rgb(%s,%s,%s"> %s </text>\n' % (
                x,
                y,
                obj[3],
                obj[2],
