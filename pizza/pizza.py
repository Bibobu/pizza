...
pizza.py switch arg(s) switch arg(s) ...
  ...
  -x raster rasmolb           load all tools except these
...
import glob
import re
from time import clock_gettime as clock
...
def trap(type, value, tback):
    ...
    if value.text[0] == "?":
        ...
        if len(words) == 1 and words[0] == "?":
            ...
            scripts = []
            for d in PIZZA_SCRIPTS[1:]:
                pizzalist = glob.glob("%s/*.py" % d)
                pizzalist.sort()
                scripts += pizzalist
            ...
            if words[1][-3:] == ".py":
                fileflag = 0
                for pizzadir in PIZZA_SCRIPTS:
                    filename = "%s/%s" % (pizzadir, words[1])
    ...
    if value.text[0] == "@":
        ...
        if words[0][1:] == "cd":
            ...
            flag = 0
            for pizzadir in PIZZA_SCRIPTS:
                fullfile = pizzadir + "/" + file
                if os.path.exists(fullfile):
                    flag = 1
                    print("Executing file:", fullfile)
                    exec(fullfile, namespace)
                    break
            if not flag:
...
if len(yes_tools) > 0:
    ...
    tools = []
    for pizzadir in PIZZA_TOOLS:
        tools += glob.glob(pizzadir + "/*.py")
        ...
        try:
            ...
            while tback:
                print("error on line %d of file %s" % (
                    tback.tb_lineno,
                    tback.tb_frame.f_code.co_filename,
                    )
                )
                tback = tback.tb_next
    elif task[0] == "command":
