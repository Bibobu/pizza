...
class mdump:
    ...
    def read_snapshot(self, f):
        try:
            ...
            if "NUMBER OF NODES" in line:
                ...
                sys.exit("unrecognized snapshot in dump file")
            n = int(f.readline())
    ...
    def map(self, *pairs):
        if len(pairs) % 2 != 0:
            sys.exit("mdump map() requires pairs of mappings")
    ...
    def vecs(self, n, *args):
        ...
        if not snap.evalues:
            sys.exit("snapshot has no element values")
        ...
        m = 0
        for i, _ in enumerate(snap.evalues):
    ...
    def reference(self):
        for i, _ in enumerate(self.snaps):
            if not self.snaps[i].nflag:
                for j in range(i, -1, -1):
                    if self.snaps[j].nflag:
                        ...
                        break
            if not self.snaps[i].nflag:
                sys.exit("no nodal coords found in previous snapshots")
            if not self.snaps[i].eflag:
                for j in range(i, -1, -1):
                    if self.snaps[j].eflag:
                        ...
                        break
            if not self.snaps[i].eflag:
                sys.exit("no elem connections found in previous snapshots")
    ...
    def viz(self, index, flag=0):
        ...
        if self.etype == "":
            ...
            etype = self.names[self.etype]

        atoms = []
        bonds = []

        # create triangle list from all elements
        # for type, either use element type (-1)
        # or user-defined column in evalues
        ...
        for i in range(snap.nelements):
            ...
            if snap.eflag == 1:
                ...
                vecs = v1 + v2 + v3
                n = normal(vecs[0:3], vecs[3:6], vecs[6:9])
                if etype == -1:
                    tris.append([element[0], element[1]] + vecs + n)
                else:
                    tris.append([element[0], evalue[etype]] + vecs + n)
            ...
            # single tet, convert to 4 tris, normals = out
                ...
                if etype == -1:
                    ...
                    tris.append([element[0], evalue[type]] + vecs + n)
                vecs = v2 + v3 + v4
                n = normal(vecs[0:3], vecs[3:6], vecs[6:9])
                if etype == -1:
                    tris.append([element[0], element[1]] + vecs + n)
...
class eselect:
    ...
    def test(self, teststr, *args):
        ...
        pattern = "\$\w*"
        patlist = re.findall(pattern, teststr)
        for item in patlist:
            name = item[1:]
